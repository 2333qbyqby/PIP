...
def test_a_pose():
    ...
    # 设置肩膀关节角度使手臂下垂
    # 根据RBDL到Bullet的映射关系，肩膀关节角度在q中的索引需要参考_rbdl_to_bullet映射
    # 左肩膀关节 (left_shoulder_ry) 在RBDL中的索引是38，在Bullet中的索引是49
    # 右肩膀关节 (right_shoulder_ry) 在RBDL中的索引是53，在Bullet中的索引是54
    q[6+38] = -np.pi/2  # 左肩膀，使手臂下垂
    q[6+53] = np.pi/2  # 右肩膀，使手臂下垂
    q[6+54] = -np.pi/2
    q[6+39] = np.pi/2
    
    set_pose(id_robot, q)
    # 应用姿势到机器人
    print("A-pose（手臂下垂）已设置完成")
    print("按Ctrl+C退出...")
